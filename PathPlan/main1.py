...
class rl_quantum_circuit:
    ...
    def compute_fidelity(self, circuit_state):
        fidelity = np.abs(np.dot(circuit_state.conj(), self.target_state))
        return fidelity

    def reset(self):
        self.quantum_circuit = tc.Circuit(self.num_qubits, inputs=self.input_state)
        # initial quantum circuit
        for i in range(self.num_qubits):
            self.quantum_circuit.ry(i, theta=np.random.uniform(0, 1))
        for i in range(self.num_qubits):
            self.quantum_circuit.cnot(i, i + 1 if i < self.num_qubits - 1 else 0)
        return self.quantum_circuit.state().ravel()
...
if __name__ == "__main__":
    train_quantum_policy(num_qubits=2, episodes=50, max_steps=50, target_fidelity=0.95)